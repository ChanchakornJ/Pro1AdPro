package se233.Project1.view;

import javafx.scene.control.Label;
import javafx.scene.input.Dragboard;
import javafx.scene.input.TransferMode;
import javafx.scene.layout.AnchorPane;

import java.io.File;
import java.util.List;
import java.util.Map;
import java.util.function.Consumer;

public class DropPane {
    private final AnchorPane dropZone;
    private final Label dropFileLabel;
    private final Map<String, String> filePathMap;

    public DropPane(AnchorPane dropZone, Label dropFileLabel, Map<String, String> filePathMap, Consumer<List<File>> onFilesDropped) {
        this.dropZone = dropZone;
        this.dropFileLabel = dropFileLabel;
        this.filePathMap = filePathMap;

        setup(onFilesDropped);
    }

    private void setup(Consumer<List<File>> onFilesDropped) {
        dropZone.setOnDragOver(event -> {
            if (event.getGestureSource() != dropZone && event.getDragboard().hasFiles()) {
                event.acceptTransferModes(TransferMode.COPY);
                dropFileLabel.setText("Release to drop files");
            }
            event.consume();
        });

        dropZone.setOnDragDropped(event -> {
            Dragboard db = event.getDragboard();
            boolean success = false;

            if (db.hasFiles()) {
                success = true;
                for (File file : db.getFiles()) {
                    filePathMap.put(file.getName(), file.getAbsolutePath());
                }
                onFilesDropped.accept(db.getFiles());
                dropFileLabel.setText(filePathMap.size() + " files loaded");
            }

            event.setDropCompleted(success);
            event.consume();
        });

        dropZone.setOnDragExited(event -> dropFileLabel.setText("Drop the file here"));
    }
}
